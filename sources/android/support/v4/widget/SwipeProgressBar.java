...
final class SwipeProgressBar {
    ...
    public void draw(Canvas canvas) {
        ...
        if (this.mRunning || this.mFinishTime > 0) {
            ...
            float f2 = ((float) ((currentAnimationTimeMillis - this.mStartTime) % 2000)) / 20.0f;
            boolean z = false;
            if (!this.mRunning) {
                f = f2;
                ...
                float f3 = (((float) ((currentAnimationTimeMillis - this.mFinishTime) % 1000)) / 10.0f) / 100.0f;
                float f4 = (float) i;
                float interpolation = INTERPOLATOR.getInterpolation(f3) * f4;
                this.mClipRect.set(f4 - interpolation, 0.0f, f4 + interpolation, (float) height);
