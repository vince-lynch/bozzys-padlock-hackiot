...
public class DrawerArrowDrawable extends Drawable {
    ...
    public void draw(Canvas canvas) {
        ...
        double height = (double) ((float) ((((int) ((((float) bounds.height()) - (3.0f * strokeWidth)) - (this.mBarGap * 2.0f))) / 4) * 2));
        double d3 = (double) strokeWidth;
        Double.isNaN(d3);
        double d4 = d3 * 1.5d;
        double d5 = (double) this.mBarGap;
        Double.isNaN(d5);
        double d6 = d4 + d5;
        Double.isNaN(height);
        canvas2.translate((float) bounds.centerX(), (float) (height + d6));
        if (this.mSpin) {
            if (this.mVerticalMirror ^ z2) {
                i2 = -1;
            }
            canvas2.rotate(lerp4 * ((float) i2));
        } else if (z2) {
